...
const App: React.FC = () => {
  ...
  return (
    ...
    <style>{`
      ...
      /* Privacy Mode Blur */
      .privacy-active .privacy-hidden {
        filter: blur(6px);
        user-select: none;
        transition: filter 0.3s ease;
      }
      .privacy-active .privacy-hidden:hover {
        filter: blur(3px); /* Slight reveal on hover if desired, or keep hard blur */
      }
    `}</style>
      <div className={`min-h-screen bg-gray-50 text-slate-700 font-sans ${isPrivacyMode ? 'privacy-active' : ''}`}>
        {!isAuthenticated ? (
